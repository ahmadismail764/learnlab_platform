...
import { studentsService, type LeaderboardStudent } from '@/services'
...
// Mock current student ID (used only for week/month demo slices).
...
export function LeaderboardPage() {
  ...
  const [allTimeEntries, setAllTimeEntries] = useState<LeaderboardEntry[] | null>(null)
  ...
  useEffect(() => {
    ...
    const loadAllTimeLeaderboard = async () => {
      ...
      try {
        const payload = await studentsService.getLeaderboard()
        if (!isMounted) return

        const normalized = [...payload]
          .sort((a, b) => b.total_xp - a.total_xp)
          .map((entry: LeaderboardStudent, index: number): LeaderboardEntry => {
            const fullName = `${entry.user.first_name} ${entry.user.last_name}`.trim()
            const displayName = fullName || entry.user.username || entry.user.email
            ...
            return {
              ...
              rank: index + 1,
              rankChange: 0,
              isCurrentUser: entry.user.email.toLowerCase() === user.email.toLowerCase(),
            }
          })

        setAllTimeEntries(normalized)
  ...
  const mockLeaderboardData: Record<TimeFilter, LeaderboardEntry[]> = useMemo(() => ({
    week: [
  ...
  const entries = useMemo(() => {
    if (timeFilter === 'allTime' && allTimeEntries) {
      return allTimeEntries
    }

    return mockLeaderboardData[timeFilter]
  }, [timeFilter, allTimeEntries, mockLeaderboardData])

  const currentStudent = entries.find((entry) => entry.isCurrentUser || entry.id === CURRENT_STUDENT_ID)
  const isCurrentInTop10 = Boolean(currentStudent && currentStudent.rank <= 10)
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      {timeFilter === 'allTime' && (isLoadingAllTime || allTimeError) && (
        <div className="text-center text-sm">
          {isLoadingAllTime ? (
            <p className="text-neutral-500 dark:text-neutral-400">{t('common:loading')}</p>
          ) : (
            <p className="text-amber-700 dark:text-amber-300">{allTimeError}</p>
          )}
        </div>
      )}
          ...
          <CardContent className="py-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-end">
                {typeof currentStudent.accuracy === 'number' ? (
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-neutral-200 dark:divide-neutral-700">
              {displayEntries.map((entry) => {
                const isCurrent = entry.isCurrentUser || entry.id === CURRENT_STUDENT_ID
                return (
                  <div
                    key={entry.id}
